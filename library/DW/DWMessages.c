...
bool dw_system_from_array(const uint16_t *arr, dw_system *out) {
	...
	out->lat = ((arr[10] & 0x0800) ? -1 : 1) *  90.0 * ((arr[11] & 0x0FFF) + ((uint32_t)(arr[10] & 0x07FF) << 12)) / (2 << 22);
	out->lon = ((arr[12] & 0x0800) ? -1 : 1) * 180.0 * ((arr[13] & 0x0FFF) + ((uint32_t)(arr[12] & 0x07FF) << 12)) / (2 << 22);
