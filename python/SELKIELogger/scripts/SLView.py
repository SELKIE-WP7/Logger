...
class IOThread:
    def __init__(
        self, filename, function, queue, onFinish=None, progress=None, stop=None
    ):
...
class SLViewGUI:
    ...
    def __init__(self, tkroot):
        ...
        self.file_label = ttk.Label(
            self.MFrame, anchor=tk.N, justify=tk.CENTER, textvariable=self.curFileName
        )
        self.file_label.grid(column=0, row=0, sticky=(tk.N, tk.W, tk.E, tk.S))

        self.progress_bar = ttk.Progressbar(
            self.MFrame, orient=tk.HORIZONTAL, mode="determinate", maximum=100
        )
        ...
        self.sourceView = tk.Listbox(
            self.sourceViewPane, listvariable=self.sourceViewItems
        )
        ...
        self.channelView = tk.Listbox(
            self.channelViewPane, listvariable=self.channelViewItems
        )
        ...
        ttk.Separator(orient="horizontal").grid(
            column=0, row=3, sticky=(tk.N, tk.W, tk.E, tk.S)
        )
        ...
        menu_file.add_command(
            label="Load...", underline=0, command=self.load, accelerator="Control + O"
        )
        # Also bind to the accelerator
        self.root.bind("<Control-o>", self.load)

        menu_file.add_command(
            label="Process", underline=1, command=self.process, state=tk.DISABLED
        )
        self.root.bind("<<ProcessingComplete>>", self.processComplete)
        self.root.bind("<<ProcessingProgress>>", self.processProgress)

        menu_file.add_command(
            label="Close", underline=0, command=self.closeFile, state=tk.DISABLED
        )
    ...
    def load(self, event=None):
        ...
        if not (ext.lower() in [".dat", ".var"]):
            tkmb.showwarning(
                ...
                message=f"{os.path.basename(root)}{ext} has an unexpected extension.",
                detail="The file will still be processed, but may not be a supported file type.",
            )
        if (ext.lower() == ".dat") and os.path.exists(root + ".var"):
            if tkmb.askyesno(
                title="Variable file",
                message="A variable information (.var) file found matching the selected data file",
                detail="Do you wish to process the variable file instead?\n\nVariable information files (.var) files are quicker to process, but only contain information about data sources and available channels. Total message counts will not be available.",
            ):
                name = root + ".var"
    ...
    def process(self, event=None):
        ...
        newIO = IOThread(
            ...
            progress=lambda x: self.root.event_generate(
                "<<ProcessingProgress>>", state=int(10000 * x)
            ),
    ...
    def processComplete(self, event=None):
        ...
        if isinstance(res, Exception):
            ...
            tkmb.showerror(
                title="Error",
                message=f"An unexpected error occurred processing {self.curFileName.get()}",
                detail=str(res),
            )
        else:
    ...
    def updateChannelView(self, event=None):
        ...
        self.channelViewItems.set(
            [f"0x{x[0]:02x}: {x[1]}" for x in list(enumerate(csmd[self.sourceID]))]
        )
    ...
    def showChannelStats(self, event=None):
        ...
        try:
            ...
            self.status.configure(
                text=f"{self.curStats[self.sourceID][channelID]:d} messages seen for source 0x{self.sourceID:02x} and channel 0x{channelID:02x}"
            )
        except Exception as e:
            self.status.configure(
                text=f"No messages seen for source 0x{self.sourceID:02x} and channel 0x{channelID:02x} - {e}"
            )
