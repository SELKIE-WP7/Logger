...
void *net_logging(void *ptargs) {
	...
	while (!shutdownFlag) {
		...
		if (!queue_push(args->logQ, sm)) {
			log_error(args->pstate, "[Network:%s] Error pushing message to queue", args->tag);
			msg_destroy(sm);
			args->returnCode = -1;
			pthread_exit(&(args->returnCode));
		}
...
bool net_connect(void *ptargs) {
	...
	int enable = 1;
	if (setsockopt(netInfo->handle, IPPROTO_TCP, TCP_NODELAY, (void *) &enable, sizeof(enable))) {
...
void *net_channels(void *ptargs) {
	...
	msg_t *m_sn = msg_new_string(netInfo->sourceNum, SLCHAN_NAME, strlen(netInfo->sourceName), netInfo->sourceName);

	if (!queue_push(args->logQ, m_sn)) {
		log_error(args->pstate, "[Network:%s] Error pushing channel name to queue", args->tag);
		msg_destroy(m_sn);
		args->returnCode = -1;
		pthread_exit(&(args->returnCode));
	}
	...
	if (!queue_push(args->logQ, m_cmap)) {
		log_error(args->pstate, "[Network:%s] Error pushing channel map to queue", args->tag);
		msg_destroy(m_cmap);
		sa_destroy(channels);
		free(channels);
		args->returnCode = -1;
...
bool net_parseConfig(log_thread_args_t *lta, config_section *s) {
	...
	if (!net) {
		log_error(lta->pstate, "[Network:%s] Unable to allocate memory for device parameters", lta->tag);
		return false;
	}
	(*net) = net_getParams();
	...
	if ((t = config_get_key(s, "host"))) {
		net->addr = config_qstrdup(t->value);
	}
	t = NULL;
	...
	if ((t = config_get_key(s, "port"))) {
		...
		if (errno) {
			log_error(lta->pstate, "[Network:%s] Error parsing source number: %s", lta->tag, strerror(errno));
			return false;
		}
	}
	t = NULL;
	...
	if ((t = config_get_key(s, "sourcenum"))) {
		...
		if (sn < 0) {
			log_error(lta->pstate, "[Network:%s] Invalid source number (%s)", lta->tag, t->value);
			return false;
		}
		if (sn < 10) {
			net->sourceNum += sn;
			...
			if (sn < SLSOURCE_EXT || sn > (SLSOURCE_EXT + 0x0F)) {
				log_warning(lta->pstate, "[Network:%s] Unexpected Source ID number (0x%02x)- this may cause analysis problems", lta->tag, sn);
			}
		}
	}
	...
	if ((t = config_get_key(s, "port"))) {
		...
		net->port = strtol(t->value, NULL, 0);
		if (errno) {
			log_error(lta->pstate, "[Network:%s] Error parsing port number: %s", lta->tag, strerror(errno));
			return false;
		}
	}
	t = NULL;
	...
	if ((t = config_get_key(s, "minbytes"))) {
		...
		if (errno) {
			log_error(lta->pstate, "[Network:%s] Error parsing minimum message size: %s", lta->tag, strerror(errno));
			return false;
		}
		if (net->minBytes <= 0) {
			log_error(lta->pstate, "[Network:%s] Invalid minimum packet size specified (%d is not greater than zero)", lta->tag, net->minBytes);
			return false;
		}
	}
	t = NULL;
	...
	if ((t = config_get_key(s, "maxbytes"))) {
		...
		if (errno) {
			log_error(lta->pstate, "[Network:%s] Error parsing maximum message size: %s", lta->tag, strerror(errno));
			return false;
		}

		if (net->maxBytes <= 0) {
			log_error(lta->pstate, "[Network:%s] Invalid maximum packet size specified (%d is not greater than zero)", lta->tag, net->maxBytes);
			return false;
		}

		if (net->maxBytes < net->minBytes) {
			log_error(lta->pstate, "[Network:%s] Invalid maximum packet size specified (%d is smaller than specified minimum packet size)", lta->tag, net->maxBytes);
			return false;
		}
	}
	t = NULL;
	...
	if ((t = config_get_key(s, "timeout"))) {
		...
		if (errno) {
			log_error(lta->pstate, "[Network:%s] Error parsing timeout: %s", lta->tag, strerror(errno));
			return false;
		}

		if (net->timeout<= 0) {
			log_error(lta->pstate, "[Network:%s] Invalid timeout value (%d is not greater than zero)", lta->tag, net->timeout);
			return false;
		}
	}
	t = NULL;
